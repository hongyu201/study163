...
# 找到当前页面免费课程
cur = driver.find_element_by_class_name('z-isCur')
lis = driver.find_elements_by_class_name('uc-course-list_itm')
lisfree = list()
for li in lis:
    if len(li.find_elements_by_class_name('u-free')) > 0:
        lisfree.append(li)
lisfree[0].find_element_by_tag_name('a').get_attribute('href')  # 课程链接
lisfree[0].find_element_by_class_name('uc-ykt-coursecard-wrap_tit_name').text  # 课程名称

# 翻页
pages = driver.find_elements_by_class_name('ux-pager_itm')
pages[0].find_element_by_tag_name('a').click()

# 登录
# thirdlinks = driver.find_elements_by_xpath("//a[@class='third-link f-fl']")
# thirdlinks[1].click()
loginbtn = driver.find_element_by_partial_link_text('登录')
loginbtn.click()
qloginimg = driver.find_element_by_id('qrlogin_img')
